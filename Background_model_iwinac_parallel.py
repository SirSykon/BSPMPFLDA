...
class Background_model(object):
    ...
    def initialize_data(self, image):                                                                                                                   # Method to initialize data.
        ...
        self.normalized_training_dataset = np.random.uniform(0, 1, 
            [self.number_of_images_for_training, self.original_image_height_with_padding, 
            self.original_image_width_with_padding, 3])                                             # We generate a matrix with noise to ensure that images frame is full of uniform noise.
    ...
    def get_initial_background_model_from_training_image_set(self):                                 # Method to get the initial background model.
        ...
        patched_dataset = None                                                                      # We create the variable.
        predicted_patches = []
        print(self.normalized_training_dataset.shape)
        for index in range(self.normalized_training_dataset.shape[0]):                              # For each image...
            for T_index in range(self.T):                                                           # For each tessera in image...
                patched_dataset = image_utils.split_in_patches_one_image_with_tessera(
                    self.normalized_training_dataset[index], self.patch_side, 
                    self.tesseras_initial_points[T_index], self.patches_per_height, self.patches_per_width)    # We split each tessera into patches.

                prediction = None
                while prediction is None:                                                           # We will try to obtain the prediction if encoder is not busy with another prediction. if it is busy, we will try again later.
                    prediction = self.encoder.predict(patched_dataset)
        ...
        #print("Time to transform data to patches: " + str(time.time()-start_time))
        aux_time=time.time()
        
        print ("Obtaining initial median and standard desviation for each component.")
        predicted_patches = np.array(predicted_patches)
        predicted_patches = np.reshape(predicted_patches,[predicted_patches.shape[0]*predicted_patches.shape[1], predicted_patches.shape[2]])
            ...
            for ii in range(self.patches_per_height):                                                           # For each patch...
                for jj in range(self.patches_per_width):
                    ...
                    for kk in range(self.narrowest_layer_size):                                                 # For each component...
                        self.MU[T_index, ii, jj, kk] = np.median(
                            predicted_patches[absolute_index::self.patches_per_image, kk])                      # We calculate "average" value   
                        self.SIGMA2[T_index, ii, jj, kk] = np.power(np.std(
                            predicted_patches[absolute_index::self.patches_per_image, kk]),2)                      # We calculate std^2 value                
                    
        #print("Time to get median and standard desviation: " + str(time.time()-aux_time))
        aux_time=time.time()
        ...
        if training:                                                                                                                # This image will be used for training.
            if self.training_done or self.processed_images >= self.number_of_images_for_training:                                   # If training has been already done.
                ...
                self.normalized_training_dataset[self.processed_images, 
                    self.padding_value:-1*(self.padding_value + self.extra_padding_height_to_cover_the_last_patch), 
                    self.padding_value:-1*(self.padding_value + self.extra_padding_width_to_cover_the_last_patch),
                    :] = image                                                                                                      # We add the image to the matrix with padding.
            ...
            for T_index in range(self.T):                                                                                                                   # For each tessera...
                ...
                #print("Time to get generate segmented grayscale image from tessera: "+ str(time.time()-aux_time))
                #print("Time to process tessera: " + str(time.time()-tessera_initial_time))        
