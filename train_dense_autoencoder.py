...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D, Reshape, Flatten, ReLU
...
TRAINING_EPOCHS = 10                                   # We define training epochs.
...
def apply_random_transformation(image):
    ...
    if rand == 1:
        return noise_utils.add_medium_low_gaussian_noise(image)
    if rand == 2:
        return noise_utils.add_medium_high_gaussian_noise(image)
    if rand == 3:
...
last_encoder_layer = Dense(16, activation="sigmoid", name = "last_encoder_layer")(x)
x = Dense(32, activation='relu')(last_encoder_layer)
x = Dense(64, activation='relu')(x)
x = Dense(128, activation='relu')(x)
x = Dense(256, activation='relu')(x)
x = Dense(512, activation='relu')(x)
x = Dense(TRAIN_PATCH_HEIGHT*TRAIN_PATCH_WIDTH*NUMBER_OF_CHANNELS, activation='relu')(x)
last_decoder_layer = Reshape((TRAIN_PATCH_HEIGHT, TRAIN_PATCH_WIDTH, NUMBER_OF_CHANNELS))(x)
...
encoded_training_data = encoder.predict(np.array(train_data))
print("Training data mean:")
print(np.mean(encoded_training_data,axis=0))
print("Training data std:")
print(np.std(encoded_training_data,axis=0))
