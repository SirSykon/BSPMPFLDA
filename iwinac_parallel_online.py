...
import Background_model_iwinac_parallel_online_update                            # pylint: disable=import-error
import Encoder
...
ENCODER_PATH = '../output/models/autoencoder_9_t_16x16/encoder.h5py'
DATASET_FOLDER = "../data/data_to_segment/"
OUTPUT_FOLDER = "../output/segmentation/test9/"
CATEGORY_TO_SEARCH = ["dynamicBackground"]
VIDEO_TO_SEARCH = ["boats", "canoe","fountain01","fountain02","overpass"]
MAX_NUMBER_OF_PARALLEL_PROCESSES = 15
...
parameters = {
    "canoe":([3,6,9],[3,6,9],[0.001,0.005]),
    "boats":([3,6,9],[3,6,9],[0.001,0.005]),
    "fountain02":([3,6,9],[3,6,9],[0.001,0.005]),
    "overpass":([3,6,9],[3,6,9],[0.001,0.005]),
    "pedestrians":([3,6,9],[3,6,9],[0.001,0.005]),
    "fountain01":([3,6,9],[3,6,9],[0.001,0.005]),
    "fall":([3,6,9],[3,6,9],[0.001,0.005])
}
    ...
    for i, img_path in enumerate(images_path):                                                      # For each image path...
        if (os.path.isfile(img_path) and (img_path[-4:]==".png" or img_path[-4:]==".jpg")):         # If it is a file and has image extensión...
            ...
            if segmented_images_count == 1:                                                         # If this is the first image...
                back_model = Background_model_iwinac_parallel_online_update.Background_model(encoder=encoder, 
            ...
            print(("This element is not an image : " + img_path + "\nProcessed {} / {} from " 
                + category + "/" + dataset).format(segmented_images_count,total_image_number))      # Print progress info.
    print(prefix + " Finish. Time: " + str(time.time()-initial_time))        
